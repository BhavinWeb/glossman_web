                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title line-height-36">{{ __('Driver List') }}</h3>
                            ...
                            <a href="{{ route('module.driver.create') }}"
                                        ...
                                        @if (userCan('category.update') || userCan('category.delete'))
                                            <td class="text-center">
                                                @if (userCan('category.update'))
                                                    ...
                                                    <a title="{{ __('edit') }}"
                                                        href="{{ route('module.driver.edit', $category->id) }}"
                                                        class="btn bg-info mr-1">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                @endif
                                                @if (userCan('category.delete'))
                                                    <form action="{{ route('module.driver.destroy', $category->id) }}"
                    ...
                    @if ($drivers->total() > $drivers->count())
    ...
    <script>
        ...
        $('.toggle-switch').change(function() {
            ...
            $.ajax({
                type: "GET",
                dataType: "json",
                url: '{{ route('module.driver.status.change') }}',
                data: {
                    'status': status,
                    'id': id
                },
