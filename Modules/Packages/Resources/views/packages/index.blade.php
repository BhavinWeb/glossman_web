            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title line-height-36">{{ __('Package List') }}</h3>
                        
                            <a href="{{ route('module.package.create') }}"
                                        ...
                                        @if (userCan('category.update') || userCan('category.delete'))
                                            <td class="text-center">
                                                @if (userCan('category.update'))
                                                    ...
                                                    <a title="{{ __('edit') }}"
                                                        href="{{ route('module.package.edit', $category->id) }}"
                                                        class="btn bg-info mr-1">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                @endif
                                                @if (userCan('category.delete'))
                                                    <form action="{{ route('module.package.destroy', $category->id) }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="10" class="text-center">
                                            <x-admin.not-found word="Service" route="module.package.create" />
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <script>
        ...
        $('.toggle-switch').change(function() {
            ...
            $.ajax({
                type: "GET",
                dataType: "json",
                url: '{{ route('module.package.status.change') }}',
                data: {
                    'status': status,
                    'id': id
                },
