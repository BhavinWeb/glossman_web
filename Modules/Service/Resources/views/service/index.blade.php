            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title line-height-36">{{ __('service_category_list') }}</h3>
                        
                            <a href="{{ route('module.service.create') }}"
                                    ...
                                    @if (userCan('category.update'))
                                        <th width="10%">
                                            ...
                                            <span data-toggle="tooltip" data-placement="top"
                                                title="It may activate or deactivate category and category wise products from website">
                                                <x-svg.info />
                                            </span>
                                        </th>
                                    @endif
                                        ...
                                        @if (userCan('category.update') || userCan('category.delete'))
                                            <td class="text-center">
                                                @if (userCan('category.update'))
                                                    ...
                                                    <a title="{{ __('edit') }}"
                                                        href="{{ route('module.service.edit', $category->id) }}"
                                                        class="btn bg-info mr-1">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                @endif
                                                @if (userCan('category.delete'))
                                                    <form action="{{ route('module.service.destroy', $category->id) }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="10" class="text-center">
                                            <x-admin.not-found word="Service" route="module.service.create" />
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <script>
        $(function() {
            ...
            function sendTaskOrderToServer(selector) {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('module.service.updateOrder') }}",
                    data: {
                        order: order,
                        _token: '{{ csrf_token() }}'
                    },
        ...
        $('.toggle-switch').change(function() {
            ...
            $.ajax({
                type: "GET",
                dataType: "json",
                url: '{{ route('module.service.status.change') }}',
                data: {
                    'status': status,
                    'id': id
                },
