                ...
                <div class="common-breadcrumbs gap-2 d-flex flex-column">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb h7">
                            ...
                            <li class="text-uppercase" aria-current="page">My Orders</li>
                        </ol>
                    </nav>
                    <h1 class="text-uppercase py-0 d-flex align-items-center display-6 fw-bold">My Orders</h1>
                </div>
            </div>
        </div>
    </div>
...
function cancelOrder(id){
    var formData = {
       ...
       var type = "POST";
       var ajaxurl = "{{route('cancel_order')}}";
       $.ajax({
           type: type,
           url: ajaxurl,
           data:formData,
           ...
           $("#list_data").html("");
            get_list();
            $("#cancel_button"+id).addClass('d-none');

            $("#order_status"+id).html('cancelled');
	...
	function loadmore(){
	  ...
	  function get_list(){
       ...
       function repeat_order(order_id){
            ...
            var formData = {
                repeat_order_id: order_id,

            };

            var type = "GET";

            var ajaxurl = '{{route("repeat_order")}}';
            ...
            $.ajax({
				...
				$("#cart_count_header").html(data);
                    alert("Your cart updated with repeated products item.");
                },
                error: function (data) {
                    console.log(data);
                }
